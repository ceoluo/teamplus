...
window.onload=function(){
	...
	methods:{
		/*loadImg(e) {
			...
			save(){
			this.$http.get('http://192.168.1.111:5000/alter_my_data',{
...
headimg(){
	...
	getinformation(){
		this.$http.get('http://192.168.1.111:5000/my_data',).then((res)=>{
	...
	/*	var formData = new FormData()
		...
		this.$http.post('http://192.168.1.111:5000/create_activity',{
			a_name:this.a_title,
	...
	seemyact(i){
		if(i==2){
			this.flag=3;
			this.myact=[];
			this.$http.get('http://192.168.1.111:5000/show_joined_activities',).then((res)=>{
				...
				for(var i=0;i<st.length;i++){
					...
					this.flag=2;
					this.myact=[];
					this.$http.get('http://192.168.1.111:5000/my_activity',).then((res)=>{
						var st=res.data;
						for(var i=0;i<st.length;i++){
							this.myact.push({
							title:st[i].a_name,
		...
		seemyteam(i){
			...
			if(i==1){
				this.flag=6;
				this.myteam=[];
				this.$http.get('http://192.168.1.111:5000/my_teams',).then((res)=>{
					var st=res.data;
					for(var i=0;i<st.length;i++){
						this.myteam.push({
				...
				this.flag=7;
				this.myteam=[];
				this.$http.get('http://192.168.1.111:5000/show_joined_teams',).then((res)=>{
					var st=res.data.data;
					for(var i=0;i<st.length;i++){
						this.myteam.push({
							content:st[i].t_introduction,
		...
		createam(){
			...
			this.$http.post('http://192.168.1.111:5000/create_team',{
				t_name:this.t_title,

...
class Scraping(object):
    ...
    def crow_check_code(self):
        ...
        ck_code = self.session.get(ck_url, headers=self.headers)
        path = os.getcwd() + '\\app\\static\\' + 'check_code.jpg'
        print(path)
        # 下载验证码
        with open(path, 'wb') as check_code:
    ...
    def crow_index(self, login_data):
        ...
        self.index_response = self.session.post(url, data=post_data, headers=self.headers).content
    ...
    def crow_date(self, login_data):
        ...
        url = "http://jwc.xhu.edu.cn/xsgrxx.aspx?xh={xh}&xm={xm}&gnmkdm={gnmkdm}".format(**data)
        user_date = self.session.get(url, headers=self.headers)
